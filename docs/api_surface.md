...
| `configure_transport_privkey_pem`, `decrypt_with_transport_priv`, `encrypt_with_transport_pub`, `TransportError`, `public_key_pem`, `reset_transport_key` (`crypto.transport`) | PUBLIC (stable) | Already in `crypto/__init__.__all__`. | none — product imports via `crypto.transport` directly in 4 files (STYLE) |
...
| `transport` (submodule, `crypto.transport`) | PUBLIC (stable, submodule-level contract) | Used as a namespace import (`from flops_agent.crypto import transport`) in one file; all its individual names are already exported. The module no longer touches the filesystem or env at all: there is no default path and no `TRANSPORT_PRIVATE_KEY_PATH` lookup inside the kernel. The embedding application calls `configure_transport_privkey_pem(pem_bytes)` once at startup — e.g. reading its own `TRANSPORT_PRIVATE_KEY_PATH`-style env var itself and passing the file contents in — and every crypto function raises `TransportError` until that call has happened. | done — key provisioning moved fully to the embedding application |
...
- (c) PRODUCT / step ③ (done): `crypto.crypto_context` (6 symbols, 17 call
  ...
  pass — the kernel now only accepts injected PEM bytes.
